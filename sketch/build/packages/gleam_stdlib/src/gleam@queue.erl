...
-opaque queue(ERE) :: {queue, list(ERE), list(ERE)}.

-spec new() -> queue(any()).
new() ->
    {queue, [], []}.

-spec from_list(list(ERH)) -> queue(ERH).
from_list(List) ->
    {queue, [], List}.

-spec to_list(queue(ERK)) -> list(ERK).
to_list(Queue) ->
...
-spec pop_front(queue(ESC)) -> {ok, {ESC, queue(ESC)}} | {error, nil}.
pop_front(Queue) ->
    case Queue of
        {queue, [], []} ->
            {error, nil};
...
-spec reverse(queue(ESH)) -> queue(ESH).
reverse(Queue) ->
...
-spec is_logically_equal(queue(ESP), queue(ESP), fun((ESP, ESP) -> boolean())) -> boolean().
...
-spec is_equal(queue(ESS), queue(ESS)) -> boolean().
is_equal(A, B) ->
    check_equal(
        erlang:element(3, A),
        erlang:element(2, A),
